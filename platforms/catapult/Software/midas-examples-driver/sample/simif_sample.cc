...
void simif_t::init_sampling(int argc, char** argv) {
	...
	last_snapshot = new uint64_t***[CHAIN_NUM];
	for (size_t t = 0; t < CHAIN_NUM; t++) {
		...
		if (chain_loop > 0) {
			last_snapshot[t] = new uint64_t**[chain_loop];
			for (size_t k = 0; k < chain_loop; k++) {
				last_snapshot[t][k] = new uint64_t*[CHAIN_SIZE[t]];
				for (size_t i = 0; i < CHAIN_SIZE[t]; i++) {
					last_snapshot[t][k][i] = new uint64_t[chain_len];
				}
			}
		}
	}
		...
		for (size_t k = 0; k < chain_loop; k++) {
			for (size_t i = 0; i < CHAIN_SIZE[t]; i++) {
				...
				for (size_t j = 0; j < chain_len; j++) {
					uint64_t tmp = read(CHAIN_ADDR[t] + i);
					printf("read out : %llx\n", tmp);
					last_snapshot[t][k][i][j] = tmp;
					snap << int_to_bin(bin, tmp, DAISY_WIDTH);
				}
			}
		}
	}
		...
		for (size_t t = 0; t < CHAIN_NUM; t++) {
			...
			if (chain_loop > 0) {
				for (size_t k = 0; k < chain_loop; k++) {
					for (size_t i = 0; i < CHAIN_SIZE[t]; i++) {
						for (size_t j = 0; j < chain_len; j++) {
							//printf("write %llx\n", last_snapshot[t][k][i][j]);
							write(CHAIN_WRADDR[t] + i, 0xffffffffffffffff);
						}
						if (t == 1) {
							write(REG_INJECT_ADDR, 1);
						}
					}
				}
			}
